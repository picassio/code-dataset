...
class AnthropicOAuthLM(BaseLM):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if not auth_token:
            raise ValueError(
                "No Anthropic OAuth token found. Run 'code-dataset auth login anthropic' or set ANTHROPIC_API_KEY"
            )
    ...
    def _convert_messages(self, messages: list[dict[str, Any]]) -> tuple[list[dict[str, Any]], str | None]:
...
def create_lm_with_oauth_fallback(model: str, api_key: str | None = None, **kwargs) -> BaseLM:
