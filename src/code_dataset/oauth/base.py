...
# Storage paths
OAUTH_DIR = Path.home() / ".code-dataset" / "oauth"
...
class OAuthProvider(ABC):
    ...
    def _start_callback_server(self) -> socketserver.TCPServer:
        ...
        server = socketserver.TCPServer(("localhost", self.config.callback_port), Handler, bind_and_activate=False)
...
def save_credentials(credentials: OAuthCredentials) -> None:
    ...
    temp_path = None
    try:
        with tempfile.NamedTemporaryFile(mode="w", dir=OAUTH_DIR, suffix=".tmp", delete=False) as f:
